...
if(isset($_POST['form_customer']))
	{
		try{
			...
			{
				throw new Exception('Retailer Name Can not be Empty');
			}
			if(empty($_POST['d_mobile']))
			{
				throw new Exception('Retailer Mobile No Can not be Empty');
			}
			if(empty($_POST['d_nid']))
			{
				throw new Exception('Retailer National Id Can not be Empty');
			}
			if(empty($_POST['d_address']))
			{
				throw new Exception('Retailer Address Can not be Empty');
			}

			$statement1=$db->prepare('update tbl_dealer_two set d_name=?,d_mobile=?,d_nid=?,d_address=? where d_id=?');
			$statement1->execute(array($_POST['d_name'],$_POST['d_mobile'],$_POST['d_nid'],$_POST['d_address'],$_POST['hidden_id_for_edit_customer']));
						
			
			$success_message1='Retailer Information Successfully Updated';
		}
		catch(Exception $e)
		{
